...
class ReactiveSite:
    ...
    @staticmethod
    def from_dict(d: Mapping[str, object], *, index_base: int = 0) -> "ReactiveSite":
        ...
        core = _normalize_indices(d.get("core_members", d.get("core_atoms", [])),
                                  field_name=f"ReactiveSite '{site_id}' core_members",
        ...
        initial = _normalize_indices(
            ...
            field_name=f"ReactiveSite '{site_id}' initial_state_members",
            index_base=index_base,
        )
        allowed = _normalize_str_set(
            d.get("allowed_state_elements", []),
            field_name=f"ReactiveSite '{site_id}' allowed_state_elements",
        )
        ...
        if max_state_members is not None and max_state_members < 0:
            raise ValueError(f"Reactive site '{site_id}' max_state_members must be >= 0.")
...
@dataclass(frozen=True)
class ReactiveSiteDefinition:
    sites: tuple[ReactiveSite, ...] = ()
    ...
    def from_mapping(
        ...
        *,
        ...
        sites = tuple(ReactiveSite.from_dict(item, index_base=index_base) for item in raw)
...
@dataclass(frozen=True)
class JointReactiveSiteReaction:
    event_id: int
    frame: int
    site_id: str
